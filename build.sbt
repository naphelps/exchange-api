...
lazy val root = (project in file("."))
  .settings(
    ...
    dockerCommands          := Seq(Cmd("ARG", "RED_HAT_UBI_TYPE=minimal RHEL_VERSION=10 BASE_IMAGE=ubi${RHEL_VERSION}-${RED_HAT_UBI_TYPE} BASE_IMAGE_REGISTRY=registry.access.redhat.com BASE_IMAGE_TAG=latest BASE_IMAGE_FROM=" ++ dockerBaseImage.value ),
                                   Cmd("FROM", "${BASE_IMAGE_FROM} AS stage0"),
                                   ...
                                   Cmd("FROM", "${BASE_IMAGE_FROM} as exchange-ubi"),
                                   ...
                                   Cmd("ARG", "BASE_IMAGE_DIGEST BASE_IMAGE_FROM IMAGE_AUTHORS=\"Open Horizon <open-horizon@lists.lfedge.org>\" IMAGE_CREATION_TIMESTAMP IMAGE_DESCRIPTION=\"Open Horizon's Exchange Server and REST API\" IMAGE_DOCUMENTATION='https://open-horizon.github.io/' IMAGE_REVISION IMAGE_SOURCE='https://github.com/open-horizon/exchange-api' IMAGE_TITLE=Exchange IMAGE_VENDOR='Open Horizon' IMAGE_VERSION"),
                                   Cmd("LABEL", "rg.opencontainers.image.authors=${IMAGE_AUTHORS} org.opencontainers.image.base.digest=${BASE_IMAGE_DIGEST} org.opencontainers.image.base.name=${BASE_IMAGE_FROM} org.opencontainers.image.created=${IMAGE_CREATION_TIMESTAMP} org.opencontainers.image.description=${IMAGE_DESCRIPTION} org.opencontainers.image.documentation=${IMAGE_DOCUMENTATION} org.opencontainers.image.licenses=Apache-2.0 org.opencontainers.image.revision=${IMAGE_REVISION} org.opencontainers.image.source=${IMAGE_SOURCE} org.opencontainers.image.title=${IMAGE_TITLE} org.opencontainers.image.vendor=${IMAGE_VENDOR} org.opencontainers.image.version=${IMAGE_VERSION}"),
